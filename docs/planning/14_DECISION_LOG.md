...
### v9 addendum — real end-to-end proof (Gallery page: build→deploy→commit→revert→redo), three more real bugs

User logged into `claude-agent` for real (own Claude Pro account, browser/device-code flow) and asked for a full live proof: have Agent Terminal build a real page, deploy it, commit it, revert it, then redo it — exercising every piece built in v7/v8 against the real host for the first time. Found and fixed three more bugs that only this kind of live test surfaces:

1. **Bind-mount path resolution across the docker.sock boundary**: `docker compose run` (both `codegen_agent` and Agent Terminal) resolves its `volumes:` bind-mount *sources* on the **host** daemon side, not inside the calling container. Running it with `cwd=/repo` (aiwebmaster's own bind-mounted view of the repo) silently created brand-new **empty** directories on the host instead of mounting the real `apps/web` etc — `claude-agent` genuinely saw an empty workspace and correctly gave up. Fixed with `-f {repo_path}/docker-compose.yml --project-directory {settings.host_repo_path}` on both call sites; new `HOST_REPO_PATH` env var / `settings.host_repo_path` carries in the real host path explicitly, since a container can't discover its own bind-mount source path from the inside. Confirmed via direct `docker exec` testing before and after the fix.
2. **Claude Code login never actually persisted**: `claude auth status` kept reporting `loggedIn: false` after a completed login. Root cause: Claude Code writes account/OAuth state to `~/.claude.json` — a *file* directly in `$HOME`, distinct from `~/.claude/` (a directory), and only the directory was volume-mounted. Fixed in `infra/claude-agent/run.sh` with a symlink set up on every container start (`~/.claude.json` → `~/.claude/.claude.json`); also stopped recommending `--entrypoint claude` for login (bypassed `run.sh` entirely, skipping the symlink) — `docker compose run --rm claude-agent auth login` now works because `run.sh` passes `auth`/`--help` straight through.
3. **`git` action failed outright — "Author identity unknown"**: `apps/aiwebmaster`'s own Dockerfile never set `git config user.name`/`user.email`. Fixed with `git config --system user.name "AIwebmaster"` / `user.email "aiwebmaster@thebradburygroup.com"` — the git-committer field only, unrelated to and doesn't replace the propose/approve/RBAC/audit trail.

All three fixed, then the full cycle re-run and confirmed for real: Claude Code read `about`/`contact` for conventions and wrote a real `apps/web/app/gallery/page.tsx` (brand colors/fonts matched correctly) → `docker` action rebuilt+redeployed `web` → `curl localhost:3002/gallery` 200 → `git` action committed → `git rm` + commit reverted, rebuild confirmed 404 → `git revert` of the revert redid it, rebuild confirmed 200 again → pushed to `origin/main` after explicit confirmation (standing rule: always confirm before push).

### v10 addendum — AIwebmaster split into its own repo, non-technical-user-safe agent surface, chat locked out of user management

Three related hardening asks after the v9 proof-of-concept, all from the same conversation:

**1. Self-revert risk closed via git submodule.** `apps/aiwebmaster` lived in this monorepo/branch — a broad revert here could have silently dragged AIwebmaster's own code backward along with everything else, since the tool that manages deployments was versioned by the same history it operates on. User chose full isolation over a lighter "pin to a ref" option. Executed as: copied current `apps/aiwebmaster` content into a fresh standalone repo (`~/project/tbg/aiwebmaster`, pushed to `git@github-tbz:shashankshekhar-ai/aiwebmaster.git` — squashed history, single "first commit", user explicitly opted for a fresh repo over `git subtree split`-preserved history), then in this monorepo: `git rm -r --cached apps/aiwebmaster` + `rm -rf apps/aiwebmaster` + `git submodule add git@github-tbz:.../aiwebmaster.git apps/aiwebmaster`. `docker-compose.yml`'s build context (`./apps/aiwebmaster`) needed **zero changes** — submodule content lands at the same path. Verified: `diff -rq` between the submodule checkout and the standalone repo came back empty, `docker compose build aiwebmaster` succeeded, the redeployed container served `/api/me` correctly. A push (or revert) on this monorepo now only ever moves the submodule *pointer* (recoverable with `git submodule update`) — AIwebmaster's own commit history is safe in its own repo. Hit the harness's own auto-mode classifier blocking `git init`/`commit`/`push` for the new repo (blocked twice, not transient) — user ran those steps themselves via the `!` prefix. Also hit a real SSH-identity mismatch on the first push attempt (`git@github.com:...` resolved to the wrong GitHub account per `~/.ssh/config`'s default `Host github.com` block; this monorepo's own `origin` already uses a dedicated `github-tbz` host alias with a different key that has org access) — fixed by pointing the new repo's `origin` at `git@github-tbz:...` to match.

**2. Non-technical-user-safe agent surface.** Confirmed by reading `sidebar.js` before this change: `NAV_ITEMS` had zero role filtering — every role saw Deploy/Git/Agent Terminal/Users/System links regardless of whether they could use them. Added a `requires` field per nav item (permission-type string or `null`), filtered out post-login in a new `filterNav()` — a `ui_editor` now only ever sees Chat + Browse. Chat's proposal schema also went from static/global to per-role: `core/aiwebmaster_agent.py::_proposable_types_for_role(role)` intersects `CHAT_PROPOSABLE_TYPES` with `auth/permissions.py::ROLE_PERMISSIONS[role]`, so a `ui_editor` chatting never gets back a `docker`/`sql` card they can only stare at and 403 on — narrower schema, not a rewrite of the (carefully-tuned) system prompt text. Action cards for `content`/`nav_link` (the types non-technical users actually see) now show a plain-English one-line summary (`friendlySummary()`) with the raw JSON payload tucked behind a "Show details" `<details>` toggle, instead of leading with JSON. Agent Terminal's raw JSONL stream — real telemetry noise (tool-call args twice over, rate-limit/thinking-token counters) — now surfaces just assistant text and a compact `"→ running: ToolName"` line by default (`parseOutputLine()`), collapsing consecutive noise lines into small "…" toggles instead of one row per line.

**3. Chat can never propose `user_management`, for any role, ever.** Confirmed first (by reading `static/users.html`) that the Users page's own form already posts directly to `/api/actions/run` with `type: "user_management"` — a human-driven form submit, not LLM-proposed — so simply removing `user_management` from `EXECUTABLE_TYPES` would have broken that legitimate admin path. Instead: new `CHAT_PROPOSABLE_TYPES = EXECUTABLE_TYPES - {"user_management"}` constrains only the LLM's tool-call schema enum (`_propose_actions_schema_for_role`); `EXECUTABLE_TYPES`/`EXECUTORS`/`ROLE_PERMISSIONS` are untouched, so the Users page keeps working exactly as before, verified via `GET /api/users` returning real data post-change. Closes a prompt-injection-to-privilege-escalation path specifically (site content flowing into `build_site_context()` trying to talk the model into proposing a new admin account) — not a capability removal. Defense in depth against a technical role reaching the same table a different way: `run_sql` now rejects any statement containing `AIWEBMASTER_USERS` (confirmed via curl: a real `UPDATE aiwebmaster_users SET role=...` gets a 422), and `code_edit`'s blocklist now includes `apps/aiwebmaster/auth/` (confirmed via curl: writing to `permissions.py` gets the same blocklist rejection as `.env`). **Accepted, not solved**: `codegen_agent`/Agent Terminal sandboxes still have full unrestricted file access within their bind mounts, including `apps/aiwebmaster/auth/` — no per-subpath mount ACL exists to carve that out without also breaking their real, currently-used ability to improve AIwebmaster's own code generally. Mitigated by the RBAC gate (infra_admin/super_admin only), §2's nav filtering (non-technical users never reach this page), and the fact that any sandbox-made change still needs a separate `docker` rebuild + `git` commit to take effect — both independently gated and audited, not a silent one-step escalation.

**Not yet verified**: full chat round-trip through a live LLM call — blocked by the configured Gemini key being out of prepaid credits (account billing, unrelated to this change); verified the schema logic directly instead (`_proposable_types_for_role` confirmed excluding `user_management` for every role via direct import) and both blocklists for real via curl. Nav filtering verified by confirming the deployed `sidebar.js` serves the new code — not yet clicked through in a browser as a non-`super_admin` user.

### v11 addendum — AIwebmaster moved out of this repo entirely (no local checkout)

The submodule from v10 still left a real checkout under `apps/aiwebmaster` (correct git behavior — a submodule's whole point is a local working copy at that path) — user wanted no trace of it in this repo's directory tree at all, not just separate history. Dropped the submodule (`git submodule deinit -f apps/aiwebmaster`, `git rm -f apps/aiwebmaster`, deleted `.gitmodules`) and moved the standalone checkout to `~/project/tbg/aiwebmaster`, a **sibling** of this repo rather than nested inside it. `docker-compose.yml`'s `aiwebmaster` build context and the `claude-agent`/`codex-agent` bind mounts all changed from `./apps/aiwebmaster` to `../aiwebmaster` — resolves to `/home/shashank/project/tbg/aiwebmaster` via `docker compose config`, confirmed. Verified for real, not just config-checked: `docker compose build aiwebmaster claude-agent codex-agent` all succeeded from the sibling path, the redeployed `aiwebmaster` container served `/api/me` correctly, and — the part that actually mattered given v9's bind-mount-resolution bug — a live `claude-agent` invocation through the same `--project-directory {HOST_REPO_PATH}` mechanism correctly listed real files from `../aiwebmaster` (not an empty directory), proving that fix generalizes past a plain `./apps/aiwebmaster` path. `apps/` now only contains `web`/`cms`/`api` — this repo has zero AIwebmaster-related files anywhere in its tree, tracked or not.
