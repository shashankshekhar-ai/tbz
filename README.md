...
- **`apps/aiwebmaster` is a git submodule** (own repo: `git@github-tbz:shashankshekhar-ai/aiwebmaster.git`) — deliberately separate so a revert/reset on this repo can never drag the ops tool's own code backward with it (a site-level revert only ever moves the submodule pointer, recoverable with `git submodule update`). Clone this repo with `git clone --recurse-submodules ...`, or run `git submodule update --init` after a plain clone — otherwise `apps/aiwebmaster` is just an empty directory. To pick up a newer AIwebmaster version: `cd apps/aiwebmaster && git pull origin main && cd ../.. && git add apps/aiwebmaster && git commit -m "chore: bump aiwebmaster"` — deliberate, not automatic.
